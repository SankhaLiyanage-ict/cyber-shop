...
$Orders = [];
$userQuery = "SELECT orders.*, users.firstName, users.lastName FROM orders INNER JOIN users ON orders.user_id = users.id WHERE orders.user_id=$UUid ";

if (isset($_GET['ord_status']) and $_GET['ord_status']) {
    $ord_status = $_GET['ord_status'];
    $userQuery = $userQuery . " AND orders.status=$ord_status ";
}
